    ...
    <div>
      ...
      <div className="overflow-x-auto rounded-xl border" style={{ borderColor: 'var(--border)' }}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {sorted.length === 0 ? (
              <tr><td colSpan={columns.length} className="px-4 py-8 text-center" style={{ color: 'var(--text-muted)' }}>لا توجد بيانات</td></tr>
            ) : sorted.map((row, i) => (
              <tr key={i} onClick={() => onRowClick?.(row)}
                className={`border-b transition-all hover:opacity-80 animate-fade-in`}
                style={{ borderColor: 'var(--border)', animationDelay: `${i * 0.03}s` }}>
                {columns.map(col => (
